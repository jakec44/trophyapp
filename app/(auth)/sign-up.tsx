...
export default function SignUpScreen() {
  ...
  const validate = (): boolean => {
    ...
    if (!trimEmail || !trimPass || !trimConfirm) {
    ...
    if (trimPass !== trimConfirm) {
      ...
      showAlert('Passwords Don\'t Match', 'Please make sure your passwords match.');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const isEmailInUse = msg.toLowerCase().includes('already') || msg.toLowerCase().includes('registered');
      const isWeakPassword = msg.toLowerCase().includes('password') || msg.toLowerCase().includes('weak');
      let userMsg = msg;
      if (isEmailInUse) userMsg = 'This email is already in use. Try signing in instead.';
      else if (isWeakPassword) userMsg = 'Password is too weak. Use at least 6 characters.';
      setError(userMsg);
      showAlert('Sign Up Failed', userMsg);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        >
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Create your Snagged account</Text>
          </View>
