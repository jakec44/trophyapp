-- Angler Rating (AR): tournament-based competitive rank. Replaces XP-based global rank for bragging rights.
-- AR rules: 1st +100, 2nd +60, 3rd +30, 4th-10th +10, below 10 -5. Server-side only, idempotent.

-- 1. Add angler_rating to profiles
...
-- Index for leaderboard ORDER BY angler_rating DESC
CREATE INDEX IF NOT EXISTS idx_profiles_angler_rating_desc ON profiles (angler_rating DESC NULLS LAST);

-- 2. Ensure profiles.state exists for local leaderboard (optional filter)
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS state text;

-- 3. finalize_tournament_cycle: award top 3 (badge, result, xp) and apply AR for ALL entrants
...
BEGIN
  ...
  -- Loop over ALL ranked entries: top 3 get badge+result+xp+AR; 4-10 get +10 AR; 11+ get -5 AR
  FOR v_ranked IN
    WITH ranked AS (
      SELECT
        e.id AS entry_id,
  ...
  LOOP
    ...
    -- AR delta: 1st +100, 2nd +60, 3rd +30, 4-10 +10, 11+ -5
    v_ar_delta := CASE
      WHEN v_place = 1 THEN 100
      WHEN v_place = 2 THEN 60
      WHEN v_place = 3 THEN 30
      WHEN v_place BETWEEN 4 AND 10 THEN 10
      ELSE -5
    END;

    -- Apply AR for every entrant (idempotent: we run once per cycle)
    UPDATE profiles
    SET angler_rating = GREATEST(0, COALESCE(angler_rating, 0) + v_ar_delta)
    WHERE id = v_ranked.user_id;

    -- Top 3 only: badge, result, xp (existing logic)
    IF v_place <= 3 THEN
      ...
      IF EXISTS (
        ...
        -- Already awarded (e.g. by claim_tournament_win); AR already applied above
        v_count := v_count + 1;
        CONTINUE;
      END IF;
...
-- 4. claim_tournament_win: when creating a new badge (awarded), also apply AR for that user (in case finalize hadn't run yet)
...
-- 5. RPC: get_angler_leaderboard(scope, state_filter, limit_n)
-- scope: 'global' | 'local'. For 'local', state_filter = user's state (e.g. 'FL'); null or '' = show all (graceful fallback).
-- Returns: rank, id, username, display_name, avatar_url, angler_rating, wins, podiums
...
GRANT EXECUTE ON FUNCTION get_angler_leaderboard(text, text, int) TO authenticated;
GRANT EXECUTE ON FUNCTION get_angler_leaderboard(text, text, int) TO anon;

COMMENT ON FUNCTION get_angler_leaderboard(text, text, int) IS 'Angler Rating leaderboard. scope=global|local, state_filter for local (e.g. FL), limit_n max rows.';

-- 6. RPC: get_angler_rank(user_id, scope, state_filter) -> single row rank for a user
...
COMMENT ON COLUMN profiles.angler_rating IS 'Competitive rating from tournament placements only. 1st +100, 2nd +60, 3rd +30, 4-10 +10, 11+ -5.';
